# root_node_backup.py  –  Unified Head Node (Backup), config-driven
#
# Selective Flooding implementation:
#   - Stays SILENT while primary is online
...
#
#    Rules:
#      - node_id format:    "BackUp-E<building_number>"
#      - primary_id format: "HeadNode-E<building_number>"  ← must match primary's node_id
#      - mqtt_pass:         same as primary's mqtt_pass
# ─────────────────────────────────────────────────────────────────────────────
...
NODE_ID         = cfg["node_id"]           # e.g. "BackUp-E3"
PRIMARY_ID      = cfg["primary_id"]        # e.g. "HeadNode-E3"
MQTT_USER       = ("Pi4-" + NODE_ID).encode()
MQTT_PASS       = cfg["mqtt_pass"].encode()
AES_KEY         = cfg.get("aes_key", "CSC2106-Group-04").encode()
BUILDING_PREFIX = cfg["building_prefix"]
